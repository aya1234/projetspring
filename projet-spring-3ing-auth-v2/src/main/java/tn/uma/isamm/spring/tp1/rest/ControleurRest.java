...
public class ControleurRest {
	...
	public List<Categorie> getCategories(){
		return metier.getCategories();
	}
	
	@GetMapping("/cat/{id}")
	public Categorie chercherCategorie(@PathVariable(name="id")long id){
	...
	public String ajouterCategorie(@RequestBody Categorie c){
		 metier.saveCategorie(c);
		 return "categorie enregistré!!!";
	}

	@PutMapping("/cat/{idcat}")
	@ResponseBody
	public void updateCategorie(@RequestBody Categorie c,@PathVariable(name="idcat" )long idcat){
		metier.updateCategorie(c,idcat);
	}
	
	@DeleteMapping("/cat/{id}")	
	public String supprimerCategorie(@PathVariable(name="id")long id){
		Categorie c = metier.getCategorieById(id);
		if(c==null)
